...
# get the load data for ba with latest data
@app.route('/get_ba_wise_hourly_load_data', methods = ['POST'])
def get_ba_wise_hourly_load_data():
	response = {"status" : False, "message" : "", "data" : {}}
	try:
		# get the request data
		data = request.get_json()[0]
		...
		#call to the common method
		response = get_load_data_latest_records(request_data)
